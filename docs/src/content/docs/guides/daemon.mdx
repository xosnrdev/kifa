...
Daemon mode reads log lines from one or more sources and persists them to disk. Each line becomes one entry in the write-ahead log.
...
Running `kifa` without a subcommand defaults to daemon mode. The following are equivalent:

```bash
kifa daemon -c config.toml
kifa -c config.toml
```

This requires at least one source to be configured in the TOML file or via environment variables.
