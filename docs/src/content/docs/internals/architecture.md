...
1. Reads the manifest to determine the checkpoint LSN.
2. Replays all WAL entries after the checkpoint LSN into the memtable.
3. Validates each entry's CRCs. Entries with invalid CRCs or incomplete trailers are discarded. They represent writes that did not complete before the crash.
4. Reports recovery statistics: entries replayed, time range, and any gaps in the LSN sequence.
